...
void spin_figure(position_f* figure,
                 part_coordinate* coordinate) {  // смена координат при повороте
  if (figure->figure != 4) {
    spin(&c->X1, &c->Y1);
    spin(&c->X2, &c->Y2);
    spin(&c->X3, &c->Y3);
...
int user_input(GameInfo_t* g, position_f* f, part_coordinate* c, int* speed) {
  int error = 0, a = 0;
  for (int i = 0; i < 10; i++) {
    int ch = 1;  // Ожидание нажатия любой клавиши для выхода
...
int lose_or_continue(GameInfo_t* g) {
  ...
  for (int i = 0; i < 10; i++) {
    if (g->field[0][i] == 1) {
      std::ofstream file("higt_score_t.txt", std::ios::trunc);
      // FILE* file = fopen("higt_score_t.txt", "w");  // Записываем данные в
      // файл fprintf(file, "%d", g->score);
      file << g->score;
      file.close();
